...
void enterDetails(player *p1) {
    system("cls");
    printf("\nEnter name: ");
    getchar();
    fgets(p1[0].name, sizeof(p1[0].name), stdin);
    printf("Enter age: ");
    scanf("%d", &p1[0].age);
    return;
}

void startGame(int *end, int *score, player *p1) {
    char array[9][50] = {
...
void addToFile(player *p1) {
    FILE *fp = fopen("GameRecords.txt", "a");
    fprintf(fp, "%d - %s - %d", p1[0].score, p1[0].name, p1[0].age);
    fclose(fp);
    free(p1);
}
...
int main() {
  ...
  int end = 1, score = 0;
  player *p1 = (player*) malloc (sizeof(player)*1);
  startGame(&end, &score, p1);
